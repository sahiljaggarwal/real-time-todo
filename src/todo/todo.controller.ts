import { Body, Controller, HttpCode, Post, Get, Put, Delete, Patch, Param } from '@nestjs/common';
...
export class TodoController {
    ...
    @Post()
    @HttpCode(201)
    async addTodo(@Body() todoDto:TodoDto):Promise<SuccessResponse<any>|ErrorResponse>{
        try {
    ...
    @Get(':id')
    @HttpCode(200)
    async TodoListById(@Param('id') id:string):Promise<SuccessResponse<any>|ErrorResponse>{
        try {
            // console.log('id',id);
    ...
    @Put(':id')
    @HttpCode(200)
    async TodoUpdate(@Param() id:string, @Body() todoDto:TodoDto):Promise<SuccessResponse<any>|ErrorResponse>{
        try {
    ...
    @Delete(':id')
    @HttpCode(200)
    async TodoDelete(@Param() id:string):Promise<SuccessResponse<any>|ErrorResponse>{
        try {
    ...
    @Patch()
    @HttpCode(200)
    async TodoIsCompleted(@Param() id:string):Promise<SuccessResponse<any>|ErrorResponse>{
        try {
