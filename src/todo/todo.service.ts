...
export class TodoService {
    constructor(@InjectModel(Todo.name) private readonly todoModel: Model<Todo>, private readonly todogateway:TodoGateway) {}
    ...
    async TodoUpdate(id:any, todoDto:TodoDto):Promise<Todo>{
        const data = await this.todoModel.findByIdAndUpdate(id, todoDto, {new:true});
        if(!data){
            throw new NotFoundException('Todo Not Found')
        }
    ...
    // delete todo
    async TodoDelete(id:any):Promise<Todo>{
        const data = await this.todoModel.findByIdAndRemove(id);
        if(!data){
            throw new NotFoundException('Todo Not Found')
        }
    ...
    async TodoIsCompleted(id:any):Promise<Todo>{
        const data = await this.todoModel.findByIdAndUpdate(id, {isComplete:true}, {new:true});
        if(!data){
            throw new NotFoundException('Todo Not Found')
        }
